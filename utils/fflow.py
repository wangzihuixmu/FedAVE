...
def read_option():
    ...
    parser.add_argument('--learning_rate_decay', help='learning rate decay for the training process;', type=float, default=0.977)
    ...
    parser.add_argument('--train_on_all', help='use both train_data and valid_data to train the model;', action="store_true", default=True)
    ...
    parser.add_argument('--mu', help='mu in fedprox', type=float, default='0.0')
    parser.add_argument('--alpha_reputatio', help='weight of last round and this round reputation in cffl', type=float, default='0.95')
    parser.add_argument('--sources', help='update the number of contribution rounds in cf_TMC_ours', type=int, default='100')
    parser.add_argument('--Gamma', help='update the gradients of rffl', type=float, default='0.5')
    parser.add_argument('--Beta', help='the ratio multiplied by the reward of rffl', type=float, default='2.0')
    parser.add_argument('--standalone', help='whether the client standalone', type=float, default='0.0')
    parser.add_argument('--num_epochs_standalone', help='number of communication rounds of standalone', type=float, default='500')
    parser.add_argument('--standalone_loss', help='standalone_loss in fedprox', type=float, default='0.0')
...
def setup_seed(seed):
    ...
    torch.manual_seed(12+seed)             # 为CPU设置随机种子
    torch.cuda.manual_seed_all(123+seed)   # 为所有GPU设置随机种子
