...
def read_data_into_dicts(task, records):
    path = '/home/wzh/experiment/FL/easyFL/fedtask/'+task+'/record'
...
def scan_records(task, header = '', filter = {}):
    path = '/home/wzh/experiment/FL/easyFL/fedtask/' + task + '/record'
    files = os.listdir(path)  #os.listdir（）返回指定路径下的文件和文件夹列表
    # check headers
...
if __name__ == '__main__':
    ...
    records = set()
    for h in headers:
        records = records.union(set(scan_records(task, h, flt))) #union()返回集合的并集，重复的元素只出现一次
